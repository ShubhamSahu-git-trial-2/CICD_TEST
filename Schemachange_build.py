...
class SnowflakeSchemachangeSession:
    ...
    _q_ch_r_checksum = "SELECT DISTINCT SCRIPT, FIRST_VALUE(CHECKSUM) OVER (PARTITION BY SCRIPT " \
                       ...
                       + "STATUS = 'Success'"
    _q_ch_fetch_ = "SELECT VERSION FROM {database_name}.{schema_name}.{table_name} WHERE SCRIPT_TYPE = 'V' and SCHEMA_NAME = '" \
                  + '_subject_area' + "' ORDER BY INSTALLED_ON DESC LIMIT 1"
    _q_sess_tag = "ALTER SESSION SET QUERY_TAG = '{query_tag}'"
    _q_ch_log = "INSERT INTO {database_name}.{schema_name}.{table_name} (VERSION,SCHEMA_NAME, DESCRIPTION, SCRIPT, SCRIPT_TYPE, " \
                + "CHECKSUM, EXECUTION_TIME, STATUS, INSTALLED_BY, INSTALLED_ON) values ('{script_version}','{schema_name_parameter}'," \
    ...
    def __init__(self, config):
        ...
        self.conArgs = {"user": config['snowflake_user'], "account": config['snowflake_account'] \
            ...
            ,"authentication_parameter": config['authentication_parameter'] }

        self.oauth_config = config['oauth_config']
        self.autocommit = config['autocommit']
        self.verbose = config['verbose']
        self.con = self.authenticate()
        self._subject_area = config['schema_name_parameter']
        self._q_ch_fetch = SnowflakeSchemachangeSession._q_ch_fetch_.replace('_subject_area',self._subject_area)
    ...
    def authenticate(self):
        ...
        authentication_parameter_final = self.conArgs['authentication_parameter']
        authentication_parameter_final = authentication_parameter_final.upper()
        #print(authentication_parameter_final)
        
        
        #if os.getenv("SNOWFLAKE_PASSWORD") is not None and os.getenv("SNOWFLAKE_PASSWORD") and authentication_parameter_final == 'snowflake':
        if os.getenv("SNOWFLAKE_PASSWORD") is not None and authentication_parameter_final == 'SNOWFLAKE' and os.getenv("SNOWFLAKE_PASSWORD"):
        ...
        elif os.getenv("SNOWFLAKE_AUTHENTICATOR") == 'externalbrowser' and authentication_parameter_final == 'EXTERNALBROWSER' and os.getenv("SNOWFLAKE_AUTHENTICATOR"):
    ...
    def apply_change_script(self, script, schema_name_parameter, script_content, change_history_table):
        ...
        frmt_args['schema_name_parameter'] = schema_name_parameter
...
def deploy_command(config):
    ...
    for script_name in all_script_names_sorted:
        ...
        if not config['dry_run']:
            schema_name_parameter = config['schema_name_parameter']
            print('script')
            print(script)
            session.apply_change_script(script, schema_name_parameter, content, change_history_table)
...
def get_schemachange_config(config_file_path, root_folder, modules_folder, snowflake_account, \
                            ...
                            dry_run, query_tag, schema_name_parameter, authentication_parameter, oauth_config, **kwargs):
    ...
    #must add input parameter here if any new parameter is to be added
    cli_inputs = {"root_folder": root_folder, \
                  ...
                  "query_tag": query_tag, "schema_name_parameter": schema_name_parameter, "authentication_parameter": authentication_parameter, "oauth_config": oauth_config}
...
def main(argv=sys.argv):
    ...
    parser_deploy.add_argument('--query-tag', type=str,
                               ...
                               required=False)
    parser_deploy.add_argument('--schema-name-parameter', type=str,
                               help='The string to provide Schema name',
                               required=True)   

    parser_deploy.add_argument('--authentication-parameter', type=str,
                               help='The string to provide authentication mechanism such as Snowflake password or externalbrowser',
                               required=True)                                  
